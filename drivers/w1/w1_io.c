...
static void w1_write_bit(struct w1_master *dev, int bit)
{
	...
	if (bit) {
	//	printk(KERN_INFO "set bit high");
		dev->bus_master->write_bit(dev->bus_master->data, 0);
		w1_delay(6);
		dev->bus_master->write_bit(dev->bus_master->data, 1);
		w1_delay(64);
	} else {
	//	 printk(KERN_INFO "set bit low");
		dev->bus_master->write_bit(dev->bus_master->data, 0);
		w1_delay(60);
		dev->bus_master->write_bit(dev->bus_master->data, 1);
		w1_delay(10);
...
void w1_write_8(struct w1_master *dev, u8 byte)
{
	...
	//	printk(KERN_INFO "bus master write 0x%x",byte);
	}
	else
		for (i = 0; i < 8; ++i) {
			if (i == 7)
				w1_pre_write(dev);
	//		printk(KERN_INFO "touch bit %d:%d",i,(byte >> i) & 0x1);
...
/*	for (i=0;i<50;i++) {
		...
		printk(KERN_INFO "%d bit read:%d",i,result);
	}*/
	w1_delay(9);
...
int w1_reset_bus(struct w1_master *dev)
{
	...
	if (dev->bus_master->reset_bus)
		 ...
		 */
		 w1_delay(460); //min required time 
//		msleep(1);
	}
