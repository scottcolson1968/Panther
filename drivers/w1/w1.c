...
int w1_timeout = 3600000; // every hour
...
//	float fSensitivityVal;
	char cVersionLetter,*readbuf=buf;
	int currentCount=0,num;
	mutex_lock(&sl->master->mutex);
	w1_reset_bus(sl->master);
	w1_write_8(sl->master, W1_READ_ROM);
	w1_read_block(sl->master, buf, 8);

	if (buf[0]==0x14) {
		printk(KERN_INFO "Teds DS2430A eeprom found.");
		w1_reset_bus(sl->master);
		...
		num=w1_read_block(sl->master, buf, 8); //read application register (basic TEDS)
...
//      buf+=8;
                num=w1_read_block(sl->master, &buf[currentCount], count-currentCount);
                if (buf[0]==0xff) {//try again
                        ...
                        w1_write_block(sl->master,wrbuf,3);
//      buf+=8;
                        num=w1_read_block(sl->master, buf,count-currentCount);
                        if(buf[0]==0xff) {
                                printk(KERN_INFO "invalid Teds data");
                                return 0;
                        }
                }
	}
	else if (buf[0]==0x2d) {
		printk(KERN_INFO "Teds DS2431 eeprom found."); 
		int offset=1; // skip over checksum
		for (i=0;i<3;i++) {
			w1_reset_bus(sl->master);
			wrbuf[0]=0xcc; //skip rom
       	 		w1_write_8(sl->master,wrbuf[0]);
			...
			wrbuf[2]=0x0;
			w1_write_block(sl->master,wrbuf,3);
			num=w1_read_block(sl->master, &buf[offset-1], 8);
			if (buf[0]==0xff) {//try again
				printk(KERN_INFO "try again");
				w1_reset_bus(sl->master);
	        		wrbuf[0]=0xcc; //skip rom
		...
		//	printk(KERN_INFO "Teds[%d]=0x%x tried=%d actual=%d",offset-1,buf[offset-1],8,num);
			offset+=8;
		}
	}
	else printk(KERN_INFO "Invalid Teds eeprom found, family 0x%x",buf[0]);
	mutex_unlock(&sl->master->mutex);
	currentCount=0;
...
//		fCutoffFreq[chan] = 0.01*pow(1+2*.001,(int)uCutOffVal);
	...
	else if (((buf[8]>>2)&0x3F) == 24)	{ // Legacy TEDS template 24 for accelerometers (not tested)
		iManufacturerID = buf[0]|((buf[1]&0xF)<<8);	// 12 bits
...
//		fCutoffFreq = 0.01*pow(1+2*.001,(int)uCutOffVal);
	...
	else if (((buf[8]>>2)&0x3F) == 25)	{  // IEEE 1451.4 TEDS template 25
...
struct w1_slave *w1_search_slave(struct w1_reg_num *id)
{
	...
	struct w1_slave *sl = NULL;
	int found = 0;
	printk (KERN_INFO "w1_search_slave");
	mutex_lock(&w1_mlock);
	list_for_each_entry(dev, &w1_masters, w1_master_entry) {
		mutex_lock(&dev->list_mutex);
		list_for_each_entry(sl, &dev->slist, w1_slave_entry) {
