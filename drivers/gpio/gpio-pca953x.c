...
extern struct gpio_descs *outcoupling;
extern struct gpio_descs *incoupling[];
...
//	return -1; breaks here
	...
	if (synchgpio==NULL) {
		...
		synchgpio = gpiod_get_array(&client->dev, cplStr, GPIOD_OUT_HIGH);
		if (IS_ERR(synchgpio)) {
			printk(KERN_INFO "unable to get synch gpio pointer err:%d\n",PTR_ERR(synchgpio));
			synchgpio = NULL;
		
		}
		else {
			val=0x54;
			printk(KERN_INFO "set sync register default 0x54");
			gpiod_set_array_value(synchgpio->ndescs,synchgpio->desc, synchgpio->info, (unsigned long *)&val);
			printk(KERN_INFO "synch-gpio ready");		
		}
	}
	if (outcoupling==NULL) {
		sprintf(cplStr, "outcoupling"); 
		outcoupling = gpiod_get_array(&client->dev, cplStr, GPIOD_OUT_LOW);
		if (IS_ERR(outcoupling)) {
			printk(KERN_INFO "unable to get drive gpio pointer err:%d\n",PTR_ERR(outcoupling));
			outcoupling = NULL;
		
		}
	}
	for(i=0;i<8;i++) {
		if (incoupling[i]==NULL) {
			sprintf(cplStr, "incoupling%d",i);
			incoupling[i] = gpiod_get_array(&client->dev, cplStr, GPIOD_OUT_LOW);
