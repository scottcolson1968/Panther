...
typedef struct {
    ...
    XDrivescale_uio_map maps[ MAX_UIO_MAPS ];
} XDrivescale_uio_info;

/***************** Variable Definitions **************************************/
static XDrivescale_uio_info uio_info;
...
static int uio_info_read_name(XDrivescale_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio0/name");//, info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XDrivescale_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio0/version");//, info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XDrivescale_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
	char sAddr[80];
	u32 iAddr;
    info->maps[n].addr = UIO_INVALID_ADDR;
    sprintf(file, "/sys/class/uio/uio%d/maps/map%d/addr", 0, n);
...
static int uio_info_read_map_size(XDrivescale_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
	char sAddr[80];
    sprintf(file, "/sys/class/uio/uio%d/maps/map%d/size", 0, n);
 /*   FILE* fp = fopen(file, "r");
    if (!fp) return -1;*/
	line_from_file(file, sAddr);
...
int XDrivescale_Initialize(XDrivescale *InstancePtr, const char* InstanceName) {
	XDrivescale_uio_info *InfoPtr = &uio_info;
 ...
 //   if (n < 0)  return XST_DEVICE_NOT_FOUND;
   // for (i = 0;  i < n; i++) {
    	strcpy(file, "/sys/class/uio/uio0/name");
 //   	strcat(file, namelist[i]->d_name);
 //   	strcat(file, "/name");
        if ((line_from_file(file, name) == 0) && (strcmp(name, "DriveScale") == 0)) {
            flag = 1;
          /*  s = namelist[i]->d_name;
            s += 3; // "uio"*/
            InfoPtr->uio_num = 0;//atoi(s);
	    
		printk(KERN_INFO "name:%s\n",name);
   //         break;
        }
		else printk(KERN_INFO "drivescale not found at uio0\n");
    ...
    sprintf(file, "/dev/uio0");//, InfoPtr->uio_num);
