			      ...
			      long mask)
{
	...
	printk(KERN_INFO "read raw drive");
	mutex_lock(&st->lock);
...
//	printk(KERN_INFO "write raw drive\n");
	switch (mask) {
	case IIO_CHAN_INFO_ENABLE:	
...
void Set_iCurrentFilter(int val) {
	if (val>3 || val<0) {
		printk(KERN_INFO "filter setting %d out of range 0 to 3",val);
		return;
	}
	if (DriveScale.IsReady == XIL_COMPONENT_IS_READY)
		XDrivescale_Set_iCurrentFilter(&DriveScale, val); 
	else printk(KERN_INFO "filter change not ready\n");
	printk (KERN_INFO "filter change %d\n",val);
}

int Get_iCurrentFilter(void) {
	int val;
	if (DriveScale.IsReady == XIL_COMPONENT_IS_READY)
		val=XDrivescale_Get_iCurrentFilter(&DriveScale); 
	else printk(KERN_INFO "filter change not ready\n");
	printk (KERN_INFO "Current filter %d\n",val);
	return val;
}

void SetDriveOffset(int val) {
	if (DriveScale.IsReady == XIL_COMPONENT_IS_READY)
		XDrivescale_Write_calibratedOffset_Words(&DriveScale, Get_iCurrentFilter(), &val, 1); 
	printk (KERN_INFO "direct write drive offset 0x%x\n",val);
}

void GetDriveCounts(int counts[]) {
	if (DriveScale.IsReady == XIL_COMPONENT_IS_READY)
		XDrivescale_Read_OffsetCounts_Words(&DriveScale, 0, counts, 4); 
	for (int i = 0; i<4; i++)
		printk(KERN_INFO "read offset counts[%d] %d\n",i,counts[i]);
}

int GetDriveOffset(void) {
	int val;
	if (DriveScale.IsReady == XIL_COMPONENT_IS_READY)
		XDrivescale_Read_calibratedOffset_Words(&DriveScale, Get_iCurrentFilter(), &val, 1); 
	printk (KERN_INFO "direct read drive offset 0x%x\n",val);
	return val;
}

void SetDriveStep(int val) {
	if (DriveScale.IsReady == XIL_COMPONENT_IS_READY)
		XDrivescale_Write_calibratedStep_Words(&DriveScale, Get_iCurrentFilter(), &val, 1); 
	printk (KERN_INFO "direct write drive step 0x%x\n",val);
}

int GetDriveStep(void) {
	int val;
	if (DriveScale.IsReady == XIL_COMPONENT_IS_READY)
		XDrivescale_Read_calibratedStep_Words(&DriveScale, Get_iCurrentFilter(), &val, 1); 
	printk (KERN_INFO "direct read drive step 0x%x\n",val);
	return val;
}

void SetDriveScale(int val) {
	if (DriveScale.IsReady == XIL_COMPONENT_IS_READY)
		XDrivescale_Set_ScaleValue(&DriveScale, val); 
	printk (KERN_INFO "direct write drive scale 0x%x\n",val);
}

int GetDriveScale(void) {
	int val;
	if (DriveScale.IsReady == XIL_COMPONENT_IS_READY)
		val = XDrivescale_Get_ScaleValue(&DriveScale); 
//	printk (KERN_INFO "direct read drive scale 0x%x\n",val);
	return val;
}
bool bDriveScaleReady=false;
void SetSampIndex(int val) {
	if (bDriveScaleReady)
		XDrivescale_Set_samp_index(&DriveScale, val); 
//	printk (KERN_INFO "direct write samp index 0x%x\n",val);
}
